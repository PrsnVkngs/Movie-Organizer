...
# title, release date, runtime, director, starrating, mpaarating, cast, reviews
#add genre
noFields = 9
prefs = ['true'] * noFields
...
def changePrefs(parent, selectedProfile, pref_strs):
    ...
    for x in range(3):
        for y in range(3):
            chk_box = ttk.Checkbutton(prefFrame, textvariable = pref_strs[tmp_counter], variable = prefs[tmp_counter], onvalue='true', offvalue = 'false')
            chk_box.grid(column = (x+1), row = (y+1), sticky = W)
            print(tmp_counter)
            tmp_counter = tmp_counter + 1
            

            
    ttk.Button(prefFrame, text='Confirm', command=lambda: push_field_INI(fieldPrefs, selectedProfile)).grid(column=1, row=5, sticky=W)
    ttk.Button(prefFrame, text='Close Window', command=lambda: close_window(prefsWindow)).grid(column=2, row=5, sticky=W)
...
def changeSrc(parent, new_label, gui_label):
    ...
    directiontext = ttk.Label(src_frame, textvariable = new_label).grid(row = 1, column = 1, sticky = W)
    ...
    change_src_button = ttk.Button(src_frame, text = "Browse", command = prompt_directory).grid(row = 1, column = 2, sticky = W)

    def confirm_dialogue():
        gui_label.set(new_label.get())
        close_window(change_source_window)
    
    confirm_button = ttk.Button(src_frame, text = "Confirm", command = confirm_dialogue).grid(row = 1, column = 3, sticky = W)
...
def changeDest(parent, new_label, gui_label):
    ...
    ttk.Label(dst_frame, textvariable = new_label).grid(row = 1, column = 1, sticky = W)
    ...
    ttk.Button(dst_frame, text = "Browse", command = prompt_directory).grid(row = 1, column = 2, sticky = W)
    
    def confirm_dialogue():
        gui_label.set(new_label.get())
        close_window(change_destination_window)

    ttk.Button(dst_frame, text = "Confirm", command = confirm_dialogue).grid(row = 1, column = 3, sticky = W)
...
#    reviewsPref = StringVar()  # 7
    ...
    pref_str_list = [StringVar('')] * noFields

    for z in range(noFields):
        pref_str_list[z].set(prefName[z])
    ...
    ttk.Button(mainframe, text="Change",
               command=lambda: changePrefs(root, userProfile.current(), pref_str_list)).grid(column=4, row=1, sticky=W)  # change prefs
